...
import { Order, Build, BuildLine } from '@/lib/types';
import {
  INVENTORY_CATEGORIES, INVENTORY_CATEGORY_MAP, describeAttributes,
...
import { X, Check, ChevronLeft, Plus, CircleAlert, Cpu, Boxes, ArrowRight, Minus } from 'lucide-react';
...
function SlotPicker({ slot, parts, stockUnits, stockLevels, builds, filters, setFilters, onPick }: {
  slot: string;
  ...
  if (catParts.length === 0) {
    return (
      <div className="max-w-2xl mx-auto text-center py-16 text-slate-400">
        <CircleAlert className="h-10 w-10 mx-auto mb-3 text-slate-200" />
        <p className="font-medium">No {cat?.label ?? slot} in stock</p>
        <p className="text-sm mt-1">Receive it via Inventory → Goods Inward, then it&apos;ll appear here.</p>
      </div>
    );
  }
