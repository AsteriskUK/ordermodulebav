    ...
    sections: [
      {
        ...
        fields: [
          { key: 'autobook.enabled', type: 'boolean', default: false,
            label: 'Book labels when orders are fetched',
            help: 'Books (never prints) a carrier label as soon as an order arrives. OFF by default — only turn this on once the carrier accounts are in production, or customers receive test labels.' },
    ...
    sections: [
      {
        ...
        fields: [
          ...
          { key: 'returns.emailLabelToCustomer', type: 'boolean', default: false,
            label: 'Email return labels to the customer by default',
            help: 'OFF by default — a sandbox label emailed to a customer is unusable and confusing.' },
        ],
      },
      {
        id: 'tickets',
